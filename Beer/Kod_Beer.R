...
# df jest ramką danych Posts forum Beer 

plotBeer1 <- function(df){
  
  # tworzymy pomocniczą ramkę danych z dat
  Posts1 <- as.Date(df$X_CreationDate)
  ...
  PostsIn2014Lato <- nrow(with(postsAsDataFrame, postsAsDataFrame[(postsAsDataFrame$Posts1 >= "2014-04-01" 
                                                                   & postsAsDataFrame$Posts1 <= "2014-09-30"), ]))
  PostsIn2014 <- nrow(with(postsAsDataFrame, postsAsDataFrame[(postsAsDataFrame$Posts1 >= "2013-10-01" 
                                                               & postsAsDataFrame$Posts1 <= "2014-03-30"), ]))
  
  # tworzymy ramke danych do generowania wykresow
  
  x <- data.table(Season = rep(c("Winter", "Summer"), 7),
...
plotBeer3 <- function(df){
  ...
  PostsIn2014 <- nrow(with(postsAsDataFrame, postsAsDataFrame[(postsAsDataFrame$Posts1 >= "2014-01-01" 
                                                               & postsAsDataFrame$Posts1 <= "2014-12-31"), ]))
  
  # tworzymy ramke danych do generowania wykresow
  
  byYear <- data.table(year = c("2014", "2015", "2016", "2017",
                                "2018", "2019", "2020"),
                       count = c(PostsIn2014, PostsIn2015, PostsIn2016, PostsIn2017,
